...
function Connector(htmlElement, canvas)
{
	...
	this.initConnector = function()
	{
		...
		{
			console.log('Unable to create connector \'' + id + '\', class is not in the correct format: connector <sourceBlockId>, <destBlockId>');
			return;
		}
		...
		if(!this.source)
		{
			console.log('cannot find source block with id \'' + splitted[1] + '\'');
			return;
		}
		...
		if(!this.destination)
		{
			console.log('cannot find destination block with id \'' + splitted[2] + '\'');
			return;
		}
